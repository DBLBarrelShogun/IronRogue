  ...
  scene.executeWithSeedOffset(() => {
    ...
    if (/\d{18}$/.test(seed)) {
      for (let s = 0; s < 3; s++) {
        ...
        starters.push(getRogueRunStarters(scene, starterSpeciesForm, startingLevel));
      }
      return;
    }
    ...
    for (let c = 0; c < starterCosts.length; c++) {
      ...
      starters.push(getRogueRunStarters(scene, starterSpecies, startingLevel));
    }
  }, 0, seed);
...
function getRogueRunStarters(scene: BattleScene, starterSpeciesForm: PokemonSpeciesForm, startingLevel: integer): Starter {
